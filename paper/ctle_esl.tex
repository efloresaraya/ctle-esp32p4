...
A natural extension is \textbf{CTLE-5b}: using $K\!=\!32$ centroids and
5-bit packed indices. The LUT grows to 128\,B per tensor, the index stream is
25\,\% wider than CTLE-4b, but the single-LUT streaming pattern is preserved
and per-group scale reads are avoided. CTLE-5b may narrow the quality gap with
INT4BW while retaining a more regular PSRAM-access pattern; its 5-bit packing
decoder overhead must be evaluated experimentally.
