...
\node[font=\small\bfseries] at (0,2.35)
    {Transient RAM per weight matrix $W$};
\node[dense] (fp32)    at (0, 1.1) {
    \textbf{Dense FP32 / INT4 / INT8}\\
    \scriptsize full dequantized $\tilde{W}$ materialized in RAM before matvec
};
\node[int8s] (int8)    at (0, 0.0) {
    \textbf{INT4 Block-wise (per-group scales)}\\
    \scriptsize dense $\tilde{W}$ \emph{plus} scale table read per row-group
};
\node[ctlef] (ctlebox) at (0,-1.1) {
    \textbf{CTLE 4-bit LUT (ours)}\\
    \scriptsize 64\,B codebook per tensor — \textbf{no dense $W$ in RAM}
};
\draw[->,thick] (fp32.east)    -- +(2.1,0)
    node[right,font=\scriptsize,align=left]{full weight\\buffer};
\draw[->,thick] (int8.east)    -- +(2.1,0)
    node[right,font=\scriptsize,align=left]{buffer\,+\,scales};
\draw[->,thick,techGreen!60!black] (ctlebox.east) -- +(2.1,0)
    node[right,font=\scriptsize,align=left,text=techGreen!60!black]
    {\textbf{zero}\\transient RAM};
\end{tikzpicture}
\caption{Transient-RAM comparison. Standard formats require a dequantized weight
matrix in SRAM/PSRAM before every matvec; INT4BW adds a per-row-group scale
table. CTLE eliminates both: a 64-byte codebook per tensor is the only metadata,
and indices stream directly into the accumulate loop (Fig.~\ref{fig:ctle_arch}).}
