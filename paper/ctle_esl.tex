...
improves quality but incurs a 31\,\% throughput penalty due to per-group scale
reads from PSRAM. A product-LUT extension, P-CTLE, removes the per-element
multiply from the inner loop, reaching 1.201\,tok/s at the same footprint as
a throughput-oriented design point for future FPGA/RISC-V micro-NPU targets.
\end{abstract}
...
LUT is 16 entries (64\,B) per matrix, small enough to remain register-resident;
...
tagged as FP32\,(0), CTLE\,(1), INT4U\,(2), INT4BW\,(3), or P-CTLE\,(4).
...
\node[ctlef] (ctlebox) at (0,-1.1) {
    ...
    \scriptsize codebook register-resident; no scale table; no dense $W$
};
...
CTLE streams nibbles against a 64-byte register-resident codebook, adding no
...
\paragraph{P-CTLE: throughput-oriented extension.}
...
without any per-element float multiply. Embedding token lookup is excluded (row
select, no matmul). On the ESP32-P4, eliminating \texttt{FMUL.S} from the inner
loop saves $\approx$4 RISC-V cycles per iteration, yielding
$\mathbf{832.5\pm1.5}$\,ms/tok ($\mathbf{1.201}$\,tok/s)---a \textbf{19.6\,\%}
gain over CTLE. The cost is activation quantization error: TinyStories PPL rises
from 15.93 to 33.14. P-CTLE thus demonstrates that CTLE-compressed tensors can
be mapped to multiplier-free product-LUT datapaths, motivating future FPGA or
RISC-V softcore micro-NPU implementations; it is not the default quality setting.

\paragraph{Tradeoff summary.}
Three distinct operating points emerge. \textbf{INT4BW} offers the best quality
(PPL\,10.71\,/\,7{,}120) but is 25\,\% larger and 31\,\% slower than CTLE.
\textbf{CTLE-PSO} gives the best balance of footprint and throughput at identical
4-bit storage: 0.965\,tok/s, 7.61\,MB, 21.5\,MB PSRAM free.
\textbf{P-CTLE} maximizes throughput (1.201\,tok/s) at the cost of activation
quantization error (+108\,\% PPL) and is intended as a hardware-architecture
design point, not the default inference mode. Rankings are stable across both
evaluation corpora and all three test prompts.
...
scale-based dequantization. The matvec kernel streams packed nibbles against a
64-byte register-resident codebook; no intermediate weight buffer is allocated.
...
by better optimizing the 9.2M-entry embedding tensor; projection matrices are
already near the K-means optimum. INT4BW achieves the best quality (PPL 10.71)
but is 31\,\% slower and 25\,\% larger due to per-group PSRAM scale reads.
P-CTLE removes the per-element multiply from the inner loop via a 256-entry
product LUT, reaching 1.201\,tok/s; it is a throughput-oriented hardware design
point, not a quality-optimal configuration.
...
A natural extension is \textbf{CTLE-5b}: $K\!=\!32$ centroids with
5-bit packed indices (128\,B LUT per tensor, 9.51\,MB total). PSO-optimized
CTLE-5b achieves PPL\,10.00 on the TinyStories probe, \emph{surpassing}
INT4BW (10.71) at the same 9.51\,MB footprint while preserving the
single-LUT streaming pattern and eliminating per-group PSRAM scale reads.
WikiText-2 PPL (8{,}226) is 15\,\% above INT4BW (7{,}120), suggesting
room for a firmware 5-bit decoder. Throughput evaluation remains future work.
